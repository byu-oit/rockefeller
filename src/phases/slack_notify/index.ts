...
export class Phase implements PhaseDeployer {
    public check(phaseConfig: SlackNotifyConfig): string[] {
        return checkPhase.checkJsonSchema(`${__dirname}/params-schema.json`, phaseConfig);
    }

    public getSecretsForPhase(phaseConfig: SlackNotifyConfig): Promise<PhaseSecrets> {
    ...
    public async deployPhase(phaseContext: PhaseContext<SlackNotifyConfig>): Promise<AWS.CodePipeline.StageDeclaration> {
    ...
    public async deletePhase(phaseContext: PhaseContext<SlackNotifyConfig>): Promise<boolean> {
